...
export function SettingsForm({ initialAnnouncement, initialContacts, initialSocials }: SettingsFormProps) {
  ...
  const [contacts, setContacts] = useState<ContactNumbersValue>({
    ...
    address: initialContacts?.address || "",
    map_iframe: initialContacts?.map_iframe || "",
  });
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="md:col-span-2">
            ...
            <input
              ...
              placeholder="Dillibazar-30, Kathmandu 44600"
              className="w-full h-9 px-3 rounded-lg bg-white border border-slate-200 text-xs text-slate-900 placeholder-slate-400 focus:outline-none focus:border-brand-primary focus:ring-1 focus:ring-brand-primary transition-colors"
            />
          </div>
          <div className="md:col-span-2">
            ...
            <textarea
              value={contacts.map_iframe}
              onChange={(e) =>
                setContacts((prev) => ({ ...prev, map_iframe: e.target.value }))
              }
              rows={2}
              placeholder="https://maps.google.com/maps?q=27.7054777,85.3258023&z=17&output=embed"
              className="w-full px-3 py-2 rounded-lg bg-white border border-slate-200 text-xs text-slate-900 placeholder-slate-400 focus:outline-none focus:border-brand-primary focus:ring-1 focus:ring-brand-primary transition-colors resize-y leading-normal font-mono"
            />
          </div>
        </div>
